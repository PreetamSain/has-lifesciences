...
export default function LogoV2({ size = 'default', theme = 'dark', className = '' }) {
  const isLarge = size === 'large';
  const isSmall = size === 'small';
  const heightClass = isLarge ? 'h-10 md:h-12' : isSmall ? 'h-6' : 'h-7 md:h-8';
  const colorClass = theme === 'light' ? 'text-[#030914]' : theme === 'blue' ? 'text-[#1566F3]' : 'text-white';
  ...
  return (
    ...
    >
      <svg 
        ...
        className={`${heightClass} w-auto ${colorClass} fill-current`}
      >
        <g>
          {/* Geometric Helix Mark */}
          <g className={theme === 'light' ? 'text-[#1566F3]' : ''}>
